...
public class CreateJobBeanMapper {
	...
	public static CreateJobPayload mapper(CreateJobBean bean) {
		...
		CustomerProduct customerProduct= new CustomerProduct(bean.getCustomer_product__dop(), bean.getCustomer_product__serial_number(), bean.getCustomer_product__imei1(), bean.getCustomer_product__imei2(), bean.getCustomer_product__popurl(), mstProductId, mstModelId);
		
		List<Problems> problemList= new ArrayList<Problems>();
		Problems problems=new Problems(problemId, bean.getProblems__remark());
		problemList.add(problems);
		
		CreateJobPayload createJobPayload = new CreateJobPayload(mstServiceLocationId,mstPlatformId,mstWarrantyStatusId,mstOemId,customer ,
				customerAddress,
				customerProduct ,
				problemList );
